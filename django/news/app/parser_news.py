...
from news.app.news import News
...
class ParserNews:
	...
	def start_parse(self):
		...
		for task in tasks:
			...
			if news_list=={}: continue
			field_id = self._news.field_id
			for i in news_list:
				links.append([news_list[i][task[0]], news_list[i][field_id]])
	...
	def _handler_rss(self, url_text, id_news):
		...
		for record in rss_data.entries:
			# Проверка наличия полей в ленте
			...
			# Парсинг информации по новости из rss
